(function(){
  ...
  function showProfile(doc){
    if(!doc) return;
    selectedProfile = doc;
    if(!profileModal || !profileName || !profileBody){
      const hash = doc.id ? `#doctor-${doc.id}` : '';
      window.location.href = `doctors.html${hash}`;
      return;
    }
    profileName.textContent = doc.name;
    const initialsText = initials(doc.name);
    profileBody.innerHTML = `
      <div class="profile-avatar-large">${initialsText}</div>
      <div class="profile-info">
        <div class="profile-line"><span class="profile-label">Specialty:</span> ${doc.specialty}</div>
        <div class="profile-line"><span class="profile-label">Hospital:</span> ${doc.hospital}</div>
        <div class="profile-line"><span class="profile-label">Location:</span> ${doc.location}</div>
        <div class="profile-line"><span class="profile-label">Languages:</span> ${doc.languages.join(', ')}</div>
        <div class="profile-line"><span class="profile-label">Experience:</span> ${doc.experienceYears} years</div>
        <div class="profile-line"><span class="profile-label">Rating:</span> ${doc.rating.toFixed(1)} / 5 stars</div>
        <div class="profile-line"><span class="profile-label">Education:</span> ${doc.education.join(', ')}</div>
        <div class="profile-line"><span class="profile-label">Bio:</span> ${doc.bio}</div>
        <div class="profile-line"><span class="profile-label">Common Conditions:</span> ${doc.conditions.join(', ')}</div>
      </div>
    `;
    profileModal.showModal();
  }

  if(profileBack && profileModal){
